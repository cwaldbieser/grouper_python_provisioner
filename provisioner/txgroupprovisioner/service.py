...
class GroupProvisionerService(Service):
    ...
    def startService(self):
        ...
        base_dn = ldap_info.get('base_dn', None)
        if base_dn is None:
            log.error("Must provide option `LDAP.base_dn`.")
            sys.exit(1)
        ...
        if (not provision_user) and (not provision_group):
            log.error("Must provision enable at least one of 'LDAP.provision_user' or `LDAP.provision_group`.")
            sys.exit(1) 
        ...
        provisioner = LDAPProvisioner(log_level=log_level, syslog_prefix=self.syslog_prefix)
        processor = LoopingCall(provisioner.process_requests, db_str, ldap_info, group_map, service_state)
        processor.start(10)
...
class LDAPProvisioner(object):
    def __init__(self, log_level='info', syslog_prefix='twisted'):
        self.log = Logger(
            observer=make_syslog_observer(
                log_level, 
                prefix=syslog_prefix))

    def process_requests(self, db_str, ldap_info, group_map, service_state):
